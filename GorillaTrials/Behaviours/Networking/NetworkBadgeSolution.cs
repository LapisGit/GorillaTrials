...
internal abstract class NetworkBadgeSolution : MonoBehaviourPunCallbacks
{
    public static NetworkBadgeSolution Instance { get; set; }
    ...
    public void Update()
    {
        ...
        if (_isPropertiesReady && _propertySetTimer <= 0)
        {
            ...
            {
                Logging.Fatal("Failed to send player properties");
                Logging.Error(ex);
            }
        }
    }
    ...
    {
        Logging.Message($"Badge sync from {player.NickName}: {string.Join(", ", properties.Keys)}");
